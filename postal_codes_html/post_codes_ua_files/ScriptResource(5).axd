...
$AA.registerAnimation = function(name, type) {
    ...
    if (type && ((type === $AA.Animation) || (type.inheritsFrom && type.inheritsFrom($AA.Animation)))) {
        ...
        throw Error.argumentType('type', type, $AA.Animation,
            'AjaxControlToolkit.Animation.registerAnimation can only register types that inherit from AjaxControlToolkit.Animation.Animation');
    }
}
...
$AA.createAnimation = function(obj, defaultTarget) {
    ...
    if (!obj || !obj.AnimationName) {
        throw Error.argument('obj', 'AjaxControlToolkit.Animation.createAnimation requires an object with an AnimationName property');
    }
    var type = $AA.__animations[obj.AnimationName.toLowerCase()];
    if (!type) {
        throw Error.argument('type', 'AjaxControlToolkit.Animation.createAnimation could not find an Animation corresponding to the name ' + obj.AnimationName);
    }
    var animation = new type();
    ...
    if (obj.AnimationChildren && obj.AnimationChildren.length) {
        if ($AA.ParentAnimation.isInstanceOfType(animation)) {
            ...
            throw Error.argument('obj', 'AjaxControlToolkit.Animation.createAnimation cannot add child animations to type ' +
                type.getName() + ' that does not derive from AjaxControlToolkit.Animation.ParentAnimation');
        }
    }
        ...
        if (setter && String.isInstanceOfType(setter) && animation[setter]) {
            ...
            if (prop.endsWith('script')) {
                ...
                if (setter && String.isInstanceOfType(setter) && animation[setter]) {
                    ...
                    throw Error.argument('obj', 'AjaxControlToolkit.Animation.createAnimation found no property corresponding to "' +
                        property + '" or "' + property.substr(0, property.length - 5) + '"');
                }
            } else if ( Sys.Debug.isDebug) {
                // Raise an error when debugging if we could not find a matching property
                throw Error.argument('obj', 'AjaxControlToolkit.Animation.createAnimation found no property corresponding to "' + property + '"');
            }
        }
    }
...
$AA.Animation.prototype = {
    ...
    set_animationTarget : function(id) {
        ...
        if (target) { 
            ...
            throw Error.argument('id', 'AjaxControlToolkit.Animation.Animation.set_animationTarget requires the ID of a Sys.UI.DomElement or Sys.UI.Control.  No element or control could be found corresponding to \'' + id + '\'');
        }
    },
...
$AA.SequenceAnimation.prototype = {
    ...
    onStep : function(percentage) {
        ...
        /// <param name="percentage" type="Number">Percentage of the animation already complete</param>
        /// <returns />
        throw Error.invalidOperation('AjaxControlToolkit.Animation.SequenceAnimation cannot be nested inside AjaxControlToolkit.Animation.ParallelAnimation');
    },
